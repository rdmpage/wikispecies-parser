	...
	foreach ($csl as $k => $v)
	{
		switch ($k)
		{
			...
			case 'publisher-place':
				$ris_values[$csl_ris_map[$k]][] = html_entity_decode(trim($v), ENT_QUOTES | ENT_HTML5, 'UTF-8');				
				break;
...
//----------------------------------------------------------------------------------------
// Convert a simple CSL object to TSV row
function csl_to_tsv($csl, $show_headings = false)
{
	$parameters = array();
	...
	foreach ($csl as $k => $v)
	{
		switch ($k)
		{
			...
			case 'URL':
			case 'DOI':
				$tsv[$csl_tsv_map[$k]] = html_entity_decode(trim($v), ENT_QUOTES | ENT_HTML5, 'UTF-8');
				break;
			...
			case 'author':
				...
				$author_string = html_entity_decode($author_string, ENT_QUOTES | ENT_HTML5, 'UTF-8');				
				$author_string = str_replace(' & ', ';', $author_string);
				
				$tsv[$csl_tsv_map[$k]] = $author_string;
